package com.dxiang.demotranslation.view;
...
import com.dxiang.demotranslation.R;
...
//				ViewHelper.setRotation(right, mRot);
	...
	public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
		Log.e(TAG, "onPageScrolled: position = "+position+";positionOffset = "+positionOffset+";positionOffsetPixels = "+positionOffsetPixels +":getCurrentItem() = "+getCurrentItem());
		if (mState == State.IDLE && positionOffset > 0) {
			oldPage = getCurrentItem();//当前的item还没有变化，（滑动中这个值不会变，只有在滑动变化定型===positionOffset==0后，这个值才会变）
