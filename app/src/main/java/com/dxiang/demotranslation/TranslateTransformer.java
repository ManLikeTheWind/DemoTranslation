 ...
 */
public class TranslateTransformer implements ViewPager.PageTransformer, ViewPager.OnPageChangeListener {
    private static final float ROT_MOD = -15f;
    private int pageIndex;
    private boolean pageChanged = true;
    public static  final String TAG=TranslateTransformer.class.getSimpleName();
    /**
    ...
    public void transformPage(View view, float position) {
        Log.e(TAG, "transformPage: position = "+position +";pageChanged = "+pageChanged);
        ViewGroup v = (ViewGroup) view.findViewById(R.id.rl);
        final TranslateScrollView mscv = (TranslateScrollView) v.findViewById(R.id.mscv);
        ...
        } else if (position==-1||position==1) {//所有效果复原----0是手机屏幕，1或-1是后一个或者前一个；离开后会==0，但是屏幕变化了，pageChanged才会为true
            bg2.setTranslationX(0);
            bg1.setTranslationX(0);
            mscv.smoothScrollTo(0, 0);
        } else if ( -1<position && position < 1){//滑动事件，在手不离开屏幕，会一直增或者加
